    ...
    <aside
      className={`${
        isOpen
          ...
          : "-ml-[8px] tracking-tight sticky top-0 z-10 h-16 text-center bg-orange-200"
      } pb-6 pt-4 `}
    >
      {isOpen && (
      ...
      <div className="lg:sticky lg:top-20 flex flex-row items-center">
        <h1
          className={`font-bold flex align-middle relative py-1 px-2 ${
            isOpen ? "hidden" : "block"
          }`}
        >
          ALMA ROTTEM
        ...
        <nav
          className={`flex ml-auto flex-${isOpen ? "col" : "row"} items-${
            isOpen ? "start" : "center"
          } justify-${isOpen ? "start" : "center"} ${
            isOpen ? "md:w-full md:items-start" : ""
          }`}
        >
          {/* Hamburger Menu */}
          {!isOpen && (
            ...
            {Object.entries(navItems).map(([path, { name, icon }]) => {
              ...
              return (
                <Link
                  ...
                  href={path}
                  onClick={closeMenu}
                  className={`transition-all hover:text-neutral-800 flex relative py-1 px-2 ${
