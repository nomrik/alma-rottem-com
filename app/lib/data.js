...
export async function fetchWorks() {
  ...
  works.sort((workA, workB) =>
    moment(workA.get("year")).isAfter(workB.get("year")) ? -1 : 1
  );

  works.forEach((work) => {
    const premiereDate =
      work.get("premiere_date") && moment(work.get("premiere_date"));
    let category = "other";
...
export async function fetchBlog() {
  ...
  const posts = await sheet.getRows();

  const postsByMonth = {};

  posts.sort((postA, postB) =>
    moment(postA.get("created_date")).isAfter(postB.get("created_date"))
      ? -1
      : 1
  );

  posts.forEach((post) => {
    console.log(post.get("created_date"));
    const month = moment(post.get("created_date")).format("MMMM YYYY");
...
export async function fetchPost(postId) {
  ...
  return posts.find((post) => post.get("id") === postId);
}
